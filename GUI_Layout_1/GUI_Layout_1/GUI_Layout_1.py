...
class program_frame(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.btnframe = ttk.Frame(self, borderwidth = 5, relief = tk.GROOVE)
        self.btnframe.grid(row = 0, column = 0, sticky = 'nsew')#.pack(side = "left", fill = "both", padx = 5, pady = 5)
        self.btn2 = ttk.Button(self.btnframe, text = "LakeShore Measure", command = lambda: self.show_frame(lakeshore_measure_frame), width = 30)
        self.btn2.grid(row = 1, column = 0)
        self.btn3 = ttk.Button(self.btnframe, text = "LakeShore Heater Config", command = lambda: self.show_frame(lakeshore_config_frame), width = 30)
        self.btn3.grid(row = 2, column = 0)
        self.btn4 = ttk.Button(self.btnframe, text = "LakeShore Input Config", command = lambda: self.show_frame(lakeshore_input_frame), width = 30)
        self.btn4.grid(row = 3, column = 0)
        self.btnEnd = ttk.Button(self.btnframe, text = "LakeShore Command Prompt", command = lambda: self.show_frame(lakeshore_command_frame), width = 30)
        self.btnEnd.grid(row = 4, column = 0)
        self.btndaq1 = ttk.Button(self.btnframe, text = "DAQ Config", command = lambda: self.show_frame(daq_config_frame), width = 30)
        self.btndaq1.grid(row = 5, column = 0)
        self.btndaq2 = ttk.Button(self.btnframe, text = "DAQ Measure", command = lambda: self.show_frame(daq_measure_frame), width = 30)
